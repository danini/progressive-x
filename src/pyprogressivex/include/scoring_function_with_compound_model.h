...
namespace progx
{
	...
	template<class _ModelEstimator>
	class EPOSScoringFunctionWithCompoundModel : public gcransac::ScoringFunction<_ModelEstimator>
	{
	protected:
		// Squared truncated threshold
		double squared_truncated_threshold;
	...
	public:
		EPOSScoringFunctionWithCompoundModel() : exponent_of_shared_score(2.5)
		{

		}

		~EPOSScoringFunctionWithCompoundModel()
		{
			...
			const bool store_inliers_ = true) const
		{
			...
			if (store_inliers_) // If the inlier should be stored, clear the variables
				inliers_.clear();
			double squared_residual, score_value; // The point-to-model residual
			std::unordered_set<std::pair<int, int>, pixel_pair_hash> pixels; // The number of pixels covered
			Eigen::MatrixXd preference_vector = Eigen::MatrixXd::Zero(point_number, 1); // Initializing the preference vector
			...
			for (size_t point_idx = 0; point_idx < point_number; ++point_idx)
			{
				...
				if (squared_residual < squared_truncated_threshold)
				{
					...
					// Select the closest pixel to the current point
					...
					// Add the pixel ot the set of covered ones
